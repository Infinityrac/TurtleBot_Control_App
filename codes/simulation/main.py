...
class default(smach.State):
    ...
    def closest_path_point(self, pos, path_points):
        ...
        return tuple(path_points[distances.argmin()])
    ...
    def clbk_laser(self, msg):
        ...
        ranges = np.append(ranges[0:91],ranges[270:]) # Close range of vision

        self.alpha = np.argmin(ranges) # Angle of the closest object
        self.dist = ranges[self.alpha] # Distance of the closest object
        # Centering values
        if self.alpha > 90:
            self.alpha -=181
    ...
    def take_action(self):
        ...
        if 0.4 < self.dist <= 1:
            linear_speed = (self.dist/1.0)*0.4
            angular_speed = (self.alpha/90.0)*1.5
    ...
    def main(self, userdata): 
        # Actualizamos los valores de las variables que se comparten entre estados.
